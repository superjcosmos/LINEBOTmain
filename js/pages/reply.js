  ...
  var rows = page.map(function(r) {
    ...
    return '<tr>' +
      ...
      '<td style="max-width:260px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">' +
        escHtml(r.content) + '</td>' +
      '<td>' + escHtml(r.start_time) + ' ~ ' + escHtml(r.end_time) + '</td>' +
      '<td>' + escHtml(r.week) + '</td>' +
      '<td><span style="color:' + statusColor + ';font-weight:600;">' + escHtml(r.status) + '</span></td>' +
...
function _renderReplyPager() {
  renderPager('replyPager', _replyFiltered.length, _replyPage, _replyPageSize, gotoReplyPage);
}
...
async function deleteReply(index) {
  await confirmAndRun('確定要刪除這筆回覆嗎？', async function() {
